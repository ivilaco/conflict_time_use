...
file write latex4 "& (1) & (2) & (3) & (4) & (5) & (6) \\ \hline" _n
	...
	* v1 - Interacción por año (Tobit) Modelo 2 y 3 Rwolf, Edad 1
	qui wyoung $ceros, cmd(tobit OUTCOMEVAR conflict_time2016 conflict_time2020 CONFLICT TIME2016 TIME2020 i.ANNO i.MUNICIPIO $controls if dummyedad==0, vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time2016 conflict_time2020) cluster(MUNICIPIO) bootstraps(100) seed(12345)
		...
		* wyoung
			...
			* 2020
			global rw2_MW: di %4.3f `= r(table)[7,4]'
			global rw2_NW1: di %4.3f `= r(table)[8,4]'
			global rw2_NW2: di %4.3f `= r(table)[9,4]'
			global rw2_NW3: di %4.3f `= r(table)[10,4]'
			global rw2_CH: di %4.3f `= r(table)[11,4]'
			global rw2_CU: di %4.3f `= r(table)[12,4]'
			
		* Sidak

			* 2016
			global rw3_MW_s: di %4.3f `= r(table)[1,6]'
			global rw3_NW1_s: di %4.3f `= r(table)[2,6]'
			global rw3_NW2_s: di %4.3f `= r(table)[3,6]'
			global rw3_NW3_s: di %4.3f `= r(table)[4,6]'
			global rw3_CH_s: di %4.3f `= r(table)[5,6]'
			global rw3_CU_s: di %4.3f `=  r(table)[6,6]'
	...
	* v2 - Interacción por año (Tobit) Modelo 5 y 6 Rwolf, Edad 2 / ACA VOY
