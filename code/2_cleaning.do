...
*******************************************
	...
	* A. Vuelvo las bases dta
	foreach file in B C I D E G H1 {
		*import delimited "${enut}/2012-2013/CAP_`file'_1213.txt", clear
		import spss using "${enut}/2012-2013/CAP_`file'_1213.sav", clear
		rename *, upper
		save "${data}/coded/enut/2012-2013/cap_`file'_1213.dta", replace
	}

	import excel "${enut}/2012-2013/Dpto y Mpio_Geog_2012_2013.xlsx", sheet("Hoja1") firstrow clear
	save "${data}/coded/enut/2012-2013/mun_1213.dta", replace

	* B. Pego las bases
	use "${data}/coded/enut/2012-2013/cap_I_1213.dta", clear

	local data "D E G H1"
	foreach file of local data {
		merge 1:1 DIRECTORIO SECUENCIA_P ORDEN using "${data}/coded/enut/2012-2013/cap_`file'_1213.dta", gen("`file'")
	}
	merge m:1 DIRECTORIO SECUENCIA_P using "${data}/coded/enut/2012-2013/cap_C_1213.dta", gen(_merge8) // Agrego C
	merge m:1 DIRECTORIO using "${data}/coded/enut/2012-2013/cap_B_1213.dta", gen(_merge6)
	merge m:1 DIRECTORIO using "${data}/coded/enut/2012-2013/mun_1213.dta", gen(_merge7)
	...
	* D. Me quedo con las variables de interés
	...
	keep COD_MPIO COD_DEPTO DIRECTORIO SECUENCIA_P ORDEN /*P7310 P1152*/ P6090 P6020 P6040 P6210 P425 P1176S1 P1176S2 P1176S3 P1176S4 P1176S5 P1176S6 P1176S7 P1176S9 P1176S10 P1176S11 P1176S12 REGION $time1 P1169S* P1174 P1172 P6020 P425 FEX_C
	rename FEX_C F_EXP
	gen ANNO = "2012-2013"
	...
	* Municipio
	...
	save "${data}/coded/enut/2012-2013/ENUT_1213.dta", replace
...
*******************************************
	...
	* A. Vuelvo las bases dta
	foreach file in B C I D E G H1 {
		*import delimited "${enut}/2016-2017/CAP_`file'_1617.csv", clear
		import excel "${enut}/2016-2017/CAP_`file'_1617.xlsx", sheet("CAP_`file'_1617") firstrow clear
		rename *, upper
		save "${data}/coded/enut/2016-2017/cap_`file'_1617.dta", replace
	}

	import excel "${enut}/2016-2017/Dpto y Mpio_Geog_2016_2017.xlsx", sheet("GEOG_2016_2017") firstrow clear
	save "${data}/coded/enut/2016-2017/mun_1617.dta", replace

	* B. Pego las bases
	use "${data}/coded/enut/2016-2017/cap_I_1617.dta", clear

	local data "D E G H1"
	foreach file of local data {
   	 	merge 1:1 DIRECTORIO SECUENCIA_P ORDEN using "${data}/coded/enut/2016-2017/cap_`file'_1617.dta", gen("`file'")
	}
	merge m:1 DIRECTORIO SECUENCIA_P using "${data}/coded/enut/2016-2017/cap_C_1617.dta", gen(_merge8)
	merge m:1 DIRECTORIO using "${data}/coded/enut/2016-2017/cap_B_1617.dta", gen(_merge6)
	merge m:1 DIRECTORIO using "${data}/coded/enut/2016-2017/mun_1617.dta", gen(_merge7)
	...
	* D. Me quedo con las variables de interés
	...
	keep COD_MPIO COD_DEPTO DIRECTORIO SECUENCIA_P ORDEN /*P7310 P1152*/ P6090 P6020 P6040 P6210 P425 P1176S1 P1176S2 P1176S3 P1176S4 P1176S5 P1176S6 P1176S7 P1176S9 P1176S10 P1176S11 P1176S12 REGION $time2 P1169S* P1174 P1172 P6020 P425 F_EXP // 272 variables
	gen ANNO = "2016-2017"
	...
	* Municipio
	...
	save "${data}/coded/enut/2016-2017/ENUT_1617.dta", replace
...
*******************************************
	...
	* The database is huge because it already comes with all chapters in one. Because I was not able to import using delimited, I transformed the .csv to .xlx and divided the database in 2 so that it takes less time to import.
	
		/* Part 1
		import excel "${enut}/2020-2021/BFENUTFINALANON.xlsx", sheet("BFENUTFINALANON") firstrow clear
		rename *, upper
		
		save "${enut}/2020-2021/BFENUTFINALANON.dta", replace 
		
		* Part 2
		import excel "${enut}/2020-2021/BFENUTFINALANON_2.xlsx", sheet("Hoja1") firstrow clear
		rename *, upper
		
		save "${enut}/2020-2021/BFENUTFINALANON_2.dta", replace 
		*/
		
	* B. Pego las bases 
	use "${enut}/2020-2021/BFENUTFINALANON_2.dta", clear
	merge 1:1 DIRECTORIO SECUENCIA_P SECUENCIA_ENCUESTA ORDEN using "${enut}/2020-2021/BFENUTFINALANON.dta"
	
	* C. Cambio de nombre las variables para homogeneizar (Nombres de este año)
	* D. Me quedo con las variables de interés
	keep COD_MPIO COD_DEPTO DIRECTORIO SECUENCIA_P ORDEN /*P7310 P1152*/ P6090 P6020 P6040 P6210 P425 P1176S1 P1176S2 P1176S3 P1176S4 P1176S5 P1176S6 P1176S7 P1176S9 P1176S10 P1176S11 P1176S12 REGION $mw $nw1 $nw2 $nw3 $ch $cu $otro $rep P1169S* P1174 P1172 P6020 P425 F_EXP
	gen ANNO = "2020-2021"
	...
	* Municipality
	tostring COD_DEPTO COD_MPIO, replace
	replace COD_MPIO = "0" + COD_MPIO if length(COD_MPIO) ==2
	replace COD_MPIO = "00" + COD_MPIO if length(COD_MPIO) ==1
	replace COD_DEPTO = "0" + COD_DEPTO if length(COD_DEPTO) ==1
	gen MUNICIPIO = COD_DEPTO + COD_MPIO
	destring MUNICIPIO REGION, replace
	drop COD_DEPTO COD_MPIO
	
	save "${data}/coded/enut/2020-2021/ENUT_2021.dta", replace 
...
*******************************************
	...
	* Estandarizando a 24h
	...
	save "${data}/coded/enut/ENUT_TOTAL.dta", replace
