	...
	foreach i in $out {
		...
		foreach a of numlist 0/1 {
			...
			forvalues n = 0/2 {		
				...
				mean `i'd [pw=F_EXP] if TIME==0 & CONFLICT==`a' & dummyedad==`n'
				scalar m1 = e(b)[1,1]
				scalar sd1 = e(sd)[1,1]
				...
				mean `i'c [pw=F_EXP] if TIME==0 & CONFLICT==`a' & dummyedad==`n'
				scalar m2 = e(b)[1,1]
				scalar sd2 = e(sd)[1,1]
...
***** Intesive - Tobit
	...
	* Age group 1
	qui wyoung $no_tobit, cmd(tobit OUTCOMEVAR conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP] if dummyedad==0, vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time) cluster(MUNICIPIO) bootstraps(100) seed(12345)
	...
	* Age group 2
	qui wyoung $no_tobit, cmd(tobit OUTCOMEVAR conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP] if dummyedad==1, vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time) cluster(MUNICIPIO) bootstraps(100) seed(12345)
	...
	* Age group 3
	qui wyoung $no_tobit, cmd(tobit OUTCOMEVAR conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP] if dummyedad==2, vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time) cluster(MUNICIPIO) bootstraps(100) seed(12345)
