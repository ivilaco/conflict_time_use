...
*******************************************
	...
	file open latex using "${sale}/reg1r_2_all.txt", write replace text
	...
	* v2 - Básica (Tobit) Modelo 4 Westfall and Young (1993)	
	qui wyoung $ceros, cmd(tobit OUTCOMEVAR conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP], vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time) cluster(MUNICIPIO) bootstraps(100) seed(12345)
	...
	* v2 - Interacción por año (Tobit) Modelo 5 y 6 Westfall and Young (1993)
	qui wyoung $ceros, cmd(tobit OUTCOMEVAR conflict_time2016 conflict_time2020 CONFLICT TIME2016 TIME2020 i.ANNO i.MUNICIPIO $controls [pw=F_EXP], vce(cluster MUNICIPIO) ll(0) ul(24)) familyp(conflict_time2016 conflict_time2020) cluster(MUNICIPIO) bootstraps(100) seed(12345)
	...
	* v3 - Básica (Dummy) Modelo 7 Rwolf
	...
	(reg NW1d conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP], cluster(MUNICIPIO)) ///
	(reg NW2d conflict_time CONFLICT TIME i.ANNO i.MUNICIPIO $controls [pw=F_EXP], cluster(MUNICIPIO)) ///
	...
	* v3 - Interacción por año (Dummy) Modelo 8 y 9 Rwolf
	...
	(reg NW1d TIME2016 TIME2020 conflict_time2016 conflict_time2020 CONFLICT i.ANNO i.MUNICIPIO $controls [pw=F_EXP], cluster(MUNICIPIO)) ///
	(reg NW2d TIME2016 TIME2020 conflict_time2016 conflict_time2020 CONFLICT i.ANNO i.MUNICIPIO $controls [pw=F_EXP], cluster(MUNICIPIO)) ///
